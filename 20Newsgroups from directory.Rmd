...
#take only words that appear in least 10 document and at most 80% of documents in the corpus
...
#we remove the empty documents from the DTM and also delete the corresponding metadata
...
processedCorpus = processedCorpus[sel_indices]
```
...
```{r}
order = table(topicModel_lda@z) #how often was a topic assigned to a token
order = order(order(-order)) #order the topics by prevelance in the corpus
...
```{r}
theta_wide <- tibble(theta_frame,'newsgroup'=unlist(shuffled_subfolder_names),'doc_length'=slam::row_sums(DTM, na.rm = TRUE))
...
```{r}
dtm_unprocessed <- DocumentTermMatrix(shuffled_corpus,control = list(removePunctuation=TRUE,tolower=TRUE,bounds =list(global=c(15,Inf))))
```
...
list_of_latex_colors = c("red","green","blue","cyan","magenta","yellow","gray","teal","violet",'lime','brown','purple','orange','pink','olive')
```
...
```{r}
for(doc_number in c(231,6234,17121,15121)){ #doc_number = 231 #17121    #15121  #first docnumber: 231, then 6234, 
...
append(vect,cat("\\textcolor{",'black','}{',list_of_words_in_doc[[i]],'} ',sep=""))}
  ...
  if(topic<=length(list_of_latex_colors)){
    append(vect,cat("\\textcolor{",list_of_latex_colors[topic],'}{',list_of_words_in_doc[[i]],'} ',sep=""))
  }

else {
append(vect,cat("\\colorbox{",list_of_latex_colors[topic-length(list_of_latex_colors)],'}{',list_of_words_in_doc[[i]],'} ',sep=""))}
}
}
sink(file=NULL)
...
#i just realized, that code is not correct
#need to use DTM instead of wordassignments for doc.length and term.frequency
...
```{r}
DFM = cast_dfm(tidy(DTM),document,term,count)
```
...
```{r}
LSAfun::plot_neighbors(Predication("drive","car",m=30,k=5,tvectors= quanteda_lsa$features)$PA,n=10,tvectors=quanteda_lsa$features,connect.lines='all')
```

```{r}
LSAfun::plot_neighbors(compose("money","chain",tvectors= quanteda_lsa$features),n=10,tvectors=quanteda_lsa$features,connect.lines='all')e
```
